        ...
        window.onload = function () {
            ...
            layui.use(['element', 'form', 'layer', 'table'], function () {
                ...
                form.on('submit(mobile_search)', function (obj) {
                    ...
                    tbRend("{{url('agen/user2/real_list')}}?account=" + account + "&review_status_s=" + review_status_s + "&user_id=" + user_id);
                    return false;
                });
                ...
                function tbRend(url) {
                    table.render({
                        ...
                        , cols: [[
                            ...
                            , {field: 'hand_pic', title: 'handheld', width: 120, templet: '#switchTpl5'}
                            , {field: 'high_status', title: 'state', width: 120, templet: '#switchTpl2'}
                            , {field: 'create_time', title: 'application time'}
                            , {fixed: 'right', title: 'operate', width: 200, align: 'center', toolbar: '#barDemo'}
                        ]]
                    });
                }

                tbRend("{{url('agen/user2/real_list')}}");
                ...
                table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'detail') { //详情
                        layer.open({
                            ...
                            , type: 2
                            , content: '{{url('agen/user2/real_info')}}?id=' + data.id
                            , area: ['900px', '700px']
                        });
                    }
