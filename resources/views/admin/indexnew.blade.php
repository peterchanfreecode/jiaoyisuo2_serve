...
<div class="winui-console layui-hide slideOutRight sp">
    ...
    <div class="winui-message">
        <div class="layui-side-scroll" id="msg_admin">
...
<script>
    layui.config({
        ...
        function get_data() {
            ...
            $.ajax({
                url: '{{url('admin/main_info')}}',
                type: 'post',
                dataType: 'json',
                success: function (res) {
                    var a = res.data.length
                    if (a !== 0) {
                        var strAudio1 = "<audio id='audioPlay1' src='./images/634.wav' hidden='true' autoplay='autoplay'>";
                        ...
                        $.each(res.data, function (index, value) {
                            html+=
                            '<div class="winui-message-item"><div class="content">'
                                +value.title+
                                '</div></div>'
                        })
                        $('.layui-side-scroll').append(html);
                        //var ti = 100;
                        $(".winui-taskbar-console").trigger("click");
                        audio1.play();
                        setTimeout(function () {
                            $(".winui-taskbar-console").trigger("click");
                        }, 15000);
