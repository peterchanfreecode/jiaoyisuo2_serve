  ...
  WinLayer.prototype.open = function (options) {
    ...
    var windowIndex = layer.open({
      ...
      full: function (window) {
        ...
        $(window)
          .find('.layui-layer-max.layui-layer-maxmin')
          .html(
            '<i class="fa fa-window-restore" style="font-size:14px;left:18px;top:10px" ><i>'
          )

        if (
          $('body')
            .attr('class')
            .replace(/\btaskbarIn.*?\b/g, '') !== $('body').attr('class')
        ) {
          //根据任务栏模式调整位置
          ...
          $(window).css(
            'height',
            parseInt($(window).css('height').replace('px', '')) -
              taskbarHeight +
              'px'
          )
          $(window)
          ...
          if (ifarme) {
            $(ifarme).css(
              'height',
              parseInt($(ifarme).css('height').replace('px', '')) -
                taskbarHeight +
                'px'
            )
          }
      ...
      restore: function (window) {
        $(window)
          ...
          .html(
            '<i class="layui-icon" style="font-size:12px;left:18px;" >&#xe626;<i>'
          )
        common.setWindowBody(window)
      },
      //拉伸回调
    ...
    if (
      (options.type || this.settings.type) == 2 &&
      (options.refresh === undefined ? this.settings.refresh : options.refresh)
    ) {
      $(windowDom)
  ...
  var common = {
    ...
    getTaskItem: function (param) {
      return typeof param === 'string'
        ? $('.winui-task-item[win-id=' + param + ']')[0]
        : isDom(param)
        ? param
        : isJquery(param)
        ? param[0]
    ...
    getWindow: function (param) {
      console.log(document.querySelectorAll('.winui-window'))
      return typeof param === 'string'
        ? $('.winui-window:has(.layui-layer-content[id=' + param + '])')[0]
        : isDom(param)
        ? param
        : isJquery(param)
        ? param[0]
    ...
    addTaskItem: function (id, title) {
      var taskItem = $(
        '<li win-id="' + id + '" class="winui-task-item">' + title + '</li>'
      )
  ...
  var call = {
    ...
    taskItemMouseUp: function (e) {
      ...
      if (e.button == 2) {
        ...
        div +=
          '<li class="closeAll">&#x5173;&#x95ED;&#x5168;&#x90E8;&#x7A97;&#x53E3;</li>' //关闭所有标签
        div +=
          '<li class="closeOther">&#x5173;&#x95ED;&#x5176;&#x4ED6;&#x7A97;&#x53E3;</li>' //关闭所有标签
        div +=
          '<li class="close"><i class="fa fa-power-off fa-fw"></i>&#x5173;&#x95ED;&#x5F53;&#x524D;&#x7A97;&#x53E3;</li>' //关闭窗口
        div += '</ul>'
        //移除之前任务项右键菜单
        ...
        $('body').append(div)
        //绑定关闭所有窗口事件
        $('.task-contextmenu .closeAll')
          .off('mouseup')
          .on('mouseup', function (e) {
            if (!e) e = window.event
            if (e.button == 0) {
              $('.winui-window.layui-layer')
                .find('.layui-layer-ico.layui-layer-close')
                .trigger('click')
            }
            $('.task-contextmenu').remove()
          })
        //绑定关闭其他窗口事件
        $('.task-contextmenu .closeOther')
          .off('mouseup')
          .on('mouseup', function (e) {
            if (!e) e = window.event
            if (e.button == 0) {
