  ...
  <div class="wrap">
    ...
    <ul class="list_content list fColor1 ft12">
      ...
      >
        ...
        <span class="width2 trs" v-if="status == 1 &&item.status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{item.fact_profits || '0' |numFilters(0)}}</span>
        <span
        ...
        <span class="width2 trs" v-if="status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{item.fact_profits || '0' |numFilters(2)}}</span>
      </li>
    </ul>
 ...
 computed: {
  ...
  methods: {
    ...
    connect() {
      ...
      that.$socket.on("closed_microorder", msg => {
