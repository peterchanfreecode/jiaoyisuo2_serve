var vue = new Vue({
    ...
    methods: {
        ...
        buyModals(types) {
            ...
            // location.href = 'dataMap.html?heyueId=' + heyueId + '&id1=' + that.legal_id + '&id2=' + that.currency_id + '&name1=' + that.legal_name + '&name2=' + that.currencyName + '&type=' + types;
            // location.href = dataMap.html?tradeId='+item.id+'&legal_id='+item.legal_id+'&currency_id='+item.currency_id+'&symbol='+item.currency_name+'/'+item.legal_name
            location.href = 'dataMap.html?tradeId=2&legal_id=3&currency_id=1&symbol=BTC/USDT';
        },
        handerTabs(type) {
            this.tabs = type;
        },
        ...
        getHistoryData() {
            ...
            initDataTokens(
                ...
                function (res) {
                    that.leverDatas.marketValue = 0.0;
                    that.leverDatas.bondTotal = 0.0;
                    that.leverDatas.transactionTotal = 0.0;
                    if (res.type !== 'ok') {
                        layer_msg(res.message);
                        return;
                    }
                    that.completeList = res.message.list;
                    that.$forceUpdate();
                }
            );
        },
        ...
        submitBtn() {
            ...
            that.leverDatas.submitMoney = that.leverDatas.lastprice;
            if (that.leverDatas.legalId != '' && that.leverDatas.currencyId) {
                var data = {
                    num: that.leverDatas.share,
                    mode: that.leverDatas.selectStatus == 1 ? 1 : 2,
                    legal_id: that.leverDatas.legalId,
                    currency_id: that.leverDatas.currencyId,
                    price: that.leverDatas.controlPrice || that.leverDatas.submitMoney,
                };
            } else {
            ...
            layer.open({
                ...
                btn2: function (index) {
                    layer_loading();
                    var url = that.type === 'buy' ? 'transaction/in' : 'transaction/out';
                    initDataTokens(
                        {
                            url: url,
                            type: 'post',
                        ...
                        function (res) {
                            ...
                            // 修改金额
                            that.listAjax()
                            // const {currencyName, share,lastprice} = that.leverDatas
                            // console.log(that.leverDatas);
                            // const usdtObj = that.bbLists.find(i=>i.currency_name=== 'USDT')
                            // if( that.type=='sell'){
                            //     usdtObj.change_balance = usdtObj.change_balance + share * lastprice
                            //     that.totalCount = that.totalCount - share
                            //  }else{
                            //      console.log( that.bbLists.find(i=>i.currency_name=== currencyName).change_balance, share);
                            //      let item = that.bbLists.find(i=>i.currency_name=== currencyName) 
                            //      item.change_balance = item.change_balance + share
                            //      // leverDatas.share * (leverDatas.controlPrice || leverDatas.submitMoney)
                            //      that.totalCount = usdtObj.change_balance - share * lastprice
                            //      console.log(usdtObj.change_balance,'==', share * lastprice,'-->', that.totalCount);
        ...
        listAjax() {
            ...
            initDataTokens(
                ...
                function (res) {
                    if (res.type == 'ok') {
                        that.bbLists = res.message.change_wallet.balance

                        // const {change_balance} = that.bbLists.find(i=>i.currency_name=== 'USDT')
                        // that.totalCount = change_balance
                        // that.leverDatas.currencyName = currencyName
        ...
        selectType(types) {
            ...
            $('.transaction-fee').text('≈ 0.0000' + that.leverDatas.legalName);
            that.leverDatas.share = '';
            const {currencyName} = this.leverDatas
            //console.log(this.leverDatas);
            if(types=='sell'){
                const {change_balance} = this.bbLists.find(i=>i.currency_name=== currencyName)
                that.totalCount = change_balance
                this.leverDatas.legalName = currencyName
            }else{
                // leverDatas.share * (leverDatas.controlPrice || leverDatas.submitMoney)
                const {change_balance} = this.bbLists.find(i=>i.currency_name=== 'USDT')
                that.totalCount = change_balance
                this.leverDatas.legalName = 'USDT'
            }
        },
        // 刷新页面
        reload() {
