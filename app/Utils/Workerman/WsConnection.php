...
class WsConnection
{
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        /*    if ($key == "BTC.USDT") {
                ...
                $daymarket_data = [
                    'change' => $this->calcPair($tick->open, $tick->close),
                    "high_change" => $this->calcPair($tick->open, $tick->high),
                    "low_change" => $this->calcPair($tick->open, $tick->low),
                    'period' => $period,
                    'id' => $tick->id,
                    'vol' => sctonum($tick->vol),
                    'amount' => sctonum($tick->amount),
                ];
                self::$btcData =  $daymarket_data;
            }*/
        if ($period == '1day') {
            //推送币种的日行情(带涨副)
...
//        echo str_repeat('=', 80) . PHP_EOL;
    ...
    private function set_redis($key, $market_data)
    {
        $hh = json_encode(['ch' => $key, 'ts' => $market_data["id"] * 1000, 'tick' => [
            'id' => $market_data["id"],
            ...
            'count' => $market_data['amount'],
        ]]);
        Redis::set($key, $hh);
    }
    private function set_redis_kline($kline_data)
    {
        $kline_data = RedisQuotation::comparison($kline_data['currency_name'], $kline_data['legal_name'], $kline_data['period'], $kline_data['time'] / 1000, $kline_data);
        RedisQuotation::set_redis_kline($kline_data['currency_name'], $kline_data['legal_name'],  $kline_data['period'], $kline_data['time'] / 1000, $kline_data);
    }
        ...
        if ($this->worker_id < 8) {
            ...
            foreach ($market_data as $period => $data) {
                ...
                foreach ($data as $key => $symbol) {
                    ...
                    $this->set_redis_kline($symbol['kline_data']);
                    echo '处理' . $key . '.' . $period . '数据' . PHP_EOL;
                    if ($period == '1min') {
                        $currency_name = strtolower($symbol['kline_data']["currency_name"] . $symbol['market_data']['quote-currency']);
                        $key = "market.{$currency_name}.kline." . $period;
                        $this->set_redis($key, $symbol['market_data']);
    ...
    protected function calcPair($open, $close)
    {
