...
class Agent extends Model
{
    ...
    public function getAgentNameEnAttribute()
    {
        $value = $this->attributes['level'];
        if ($value > 0) {
            $p ="Tier ". $value . ' agent';
    ...
    {
        return self::where("id", self::getAgentId())->where("status", 1)->first();
    }
    ...
    //获取用户的代理商关系
    public static function agentPath($agent_note_id = 1)
    {
        $agent = self::find($agent_note_id);
        return $agent ? $agent->agent_path : 1;
    }

    //获取用户的代理商关系
    public static function update_agentPath($agent_path, $parent_agent_id, $agent_id)
    {
        //替换更改的代理线
        $arr = explode(",", $agent_path);
        $str = "";
        if ($arr) {
