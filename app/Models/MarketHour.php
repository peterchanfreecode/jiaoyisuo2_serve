...
//            echo 'get and set ' . json_encode($market_data);
    ...
    public static function getEsearchMarket($base_currency, $quote_currency, $peroid, $from, $to)
    {
        ...
        $size > 500 && $size = 500;
        if ($match->market_from == 1 || $match->market_from == 2 || $match->market_from == 4) {
            if ($match->market_from == 1) {
                $base_currencys = $currency_model->type;
            }
            $key = "huobi:get:" . strtolower($base_currency) . '_' . strtolower($quote_currency) . "_" . $peroid;
            if (!redis::get($key)) {
                ...
                $res = RedisQuotation::get_redis_kline_all($base_currency, $quote_currency, $peroid, $from, $to);
            }
        } else {
            $res = RedisQuotation::get_redis_kline_all($base_currency, $quote_currency, $peroid, $from, $to);
        }
    ...
    {
        $market = MarketHour::getLastEsearchMarket($base_currency, $quote_currency, '1min');
        ...
        $currency_match = CurrencyMatch::where('currency_id', $currency->id)
            ...
            ->first();
        $high = $market['high'] > $price ? $market['high'] : $price;
        $low = $market['low'] < $price ? $market['low'] : $price;
        $kline_data = [
            'type' => 'kline',
            'period' => '1min',
    ...
    protected  static function curl_get($url, $gzip = false)
    {
        $curl = curl_init($url);
