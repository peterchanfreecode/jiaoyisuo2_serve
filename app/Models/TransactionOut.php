...
class TransactionOut extends Model
{
    ...
    public static function transaction($price,$num, $user, $wallet, $legal_id, $currency_id)
    {
        ...
        change_wallet_balance($out_currency, 2, -$num, AccountLog::TRANSACTIONIN, '币币交易扣除余额');
        $cny = bc_mul($num, $price, 5);
        change_wallet_balance($out_legal, 2, $cny, AccountLog::TRANSACTIONIN, '币币交易增加余额');
        $rate = Setting::getValueByKey("COIN_TRADE_FEE");
        if($rate>0){
            //扣除手续费
            $out_legal = UsersWallet::where("user_id", $user->id)
                ->where("currency", $legal_id)
                ->first();
            $rate_cny = bc_mul($cny, $rate, 5);
            change_wallet_balance($out_legal, 2, -$rate_cny, AccountLog::TRANSACTION_FEE, '币币交易扣除手续费');
        }
        //插入完成记录
        $complete = new TransactionComplete();
        $complete->way = 1;
