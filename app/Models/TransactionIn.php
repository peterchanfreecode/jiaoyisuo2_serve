...
class TransactionIn extends Model
{
    ...
    public static function transaction($price, $num, $user, $legal_id, $currency_id)
    {
        ...
        change_wallet_balance($to_legal, 2, -$cny, AccountLog::TRANSACTIONIN, '币币交易扣除余额');
        change_wallet_balance($to_currency, 2, $num, AccountLog::TRANSACTIONIN, '币币交易增加余额');
        ...
        if($rate>0){
            ...
            $to_currency = UsersWallet::where("user_id", $user->id)
                ->where("currency", $currency_id)
                ->first();
            $rate_cny = bc_mul($num, $rate, 5);
            change_wallet_balance($to_currency, 2, -$rate_cny, AccountLog::TRANSACTION_FEE, '币币交易扣除手续费');
        }
