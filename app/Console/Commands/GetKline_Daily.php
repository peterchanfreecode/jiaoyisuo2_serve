...
class GetKline_Daily extends Command
{
    ...
    private function sendKine($currency, $peroid, $open, $close, $low, $high, $vol, $first_stamp)
    {
        ...
        CurrencyQuotation::getInstance($currency->legal_id, $currency->currency_id)
            ->updateData([
                'change' => $change,
                'now_price' => $close,
                'volume' => sctonum($vol),
                ...
                foreach ($currencys as $currency) {
                    ...
                    if ($info) {
                        ...
                        if ($time === Robot::getNowTime($peroid) / 1000 || $time < Robot::getNowTime($peroid) / 1000) {
                          /*  if ($time < Robot::getNowTime($peroid) / 1000) {
                                $last_time = strtotime('+1 day', $time);
                            } else {
                                $last_time = $time;
                            }*/
                            $last_time  = Robot::getNowTime($peroid) / 1000;
                            $start = $this->getInfo($last_time, strtotime('+1 day', $last_time), $currency_name, $legal_name);
                            ...
                            $this->info('已设置一条信息新增' . $currency["id"]);
                        } else {
                            $this->info('超出当前时间戳');
                        }
                    } else {
                    ...
                    if ($result == false) {
                        ...
                        /*    $time = $info['id'] / 1000;*/
                            ...
                            $start = $this->getInfo($time, strtotime('+1 day', $time), $currency_name, $legal_name);
                            $open = sprintf('%.6f', $start['open']??$info["open"]);
                            $close = sprintf('%.6f', $start['close']??$info["open"]);
                            $high = sprintf('%.6f', $start['high']??$info["open"]);
                            $low = sprintf('%.6f', $start['low']??$info["open"]);
                            ...
                            $this->sendKine($currency, $peroid, $open, $close, $low, $high, $vol, $first_stamp);
                            $this->info('已设置第一条信息新增' . $currency["id"]);
                        }
                    }
                }
