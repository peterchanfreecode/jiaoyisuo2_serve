...
use App\Models\{AccountLog, Agent, Currency, Token, UserCashInfo, UserReal, Users, UsersWalletOut};
...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("agent.user.index", ["is_admin" => $self["is_admin"], 'parent_id' => $parent_id, 'legal_currencies' => $legal_currencies]);
    }
    ...
    {
        $self = Agent::getAgent()->toArray();
        return view("agent.salesmen.index", ["is_admin" => $self["is_admin"], "id" => $self["id"]]);
    }
    ...
    public function update_agent()
    {
        ...
        return view("agent.salesmen.update_agent", ['d' => $list, "id" => $data["id"]]);
    }
    ...
    public function update_agent_add(Request $request)
    {
        ...
        if (!$agent_info) {
            return $this->error("选择的代理不存在");
        }
        $info = Agent::getAgentById($id);
        if (!$info) {
            return $this->error("操作的代理不存在");
        }
    ...
    {
        $user_id = request()->get('id', 0);
        return view("agent.user.modify_password", ['user_id' => $user_id]);
    }
    ...
    public function user_mic(Request $request)
    {
        ...
        return view("agent.user.mic", ['list' => $list, "user_id" => $user_id]);
    }
